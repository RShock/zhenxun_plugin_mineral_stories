...
from .player_handler import register_new_player, get_player, buy_item_handle
...
from .shop_handler import get_store_list
...
async def _(event: GroupMessageEvent):
    await go_home_handle(event, force_go_home=True)
...
async def _(event: GroupMessageEvent, state: T_State, p: str = ArgStr("choose")):
    ...
    if p >= len(state["tmp"]) or p < 0:
        await set_out.finish("输入的数字范围不对")
    state["item_name"] = state["tmp"][p]


@game_store.got("choose2", "要买几个呢？(输入数字编号)")
async def _(event: GroupMessageEvent, state: T_State, num: str = ArgStr("choose2")):
    if not num.isdigit():
        await set_out.finish("输入的格式不对，请输入数字")
    num = int(num)
    if num > len(state["tmp"]) or num < 0:
        await set_out.finish("输入的数字范围不对")
    await buy_item_handle(event, state["item_name"], num)
